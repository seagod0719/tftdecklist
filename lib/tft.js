...
export function decorateComp(comp, lookup) {
  return { ...comp, units: comp.unitIds.map(id => ({ id, name: lookup.units.get(id)?.name || id, cost: lookup.units.get(id)?.cost ?? null,
    icon: `https://tftable.cc/${lookup.set.replace('TFTSet', 'set')}/avatar-webp/${encodeURIComponent(lookup.units.get(id)?.assetNames?.[0] || id)}.webp?v=5`
  })), ...createTeamCode(comp.unitIds, lookup) };
}
